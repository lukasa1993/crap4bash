...
DEFAULT_COVERAGE = Path('target/coverage')
DEFAULT_TEST_COMMAND = 'kcov target/coverage bats tests'
SAFE_GENERATED_NAMES = {"target", "coverage", ".coverage", "build", ".build"}


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description='Calculate strict CRAP scores for Bash functions.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if not args.no_test:
            ...
            report = _resolve_swift_report(completed.stdout or "", coverage) if 'bash' == "swift" else discover_coverage_report(coverage)
        ...
        print(f"crap4bash: {error}", file=sys.stderr)
        return 1
